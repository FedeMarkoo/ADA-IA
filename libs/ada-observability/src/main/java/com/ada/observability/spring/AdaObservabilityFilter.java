...
public class AdaObservabilityFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    if (properties.getIgnoredPaths().contains(request.getRequestURI())) {
    ...
    TraceContext trace =
        TraceContext.create(
            ...
            correlation,
            request.getHeader("x-ada-caller"),
            request.getHeader("x-ada-ini-caller"));
    try (var scope =
        observability.start(request.getMethod() + " " + request.getRequestURI(), "REST", trace)) {
      scope.event("originIp", request.getRemoteAddr());
      try {
        chain.doFilter(request, response);
        scope.status(response.getStatus());
      } catch (RuntimeException | IOException | ServletException error) {
        scope.failure(error);
        scope.status(response.getStatus());
        throw error;
      }
