...
def detect_burst_groups(files):
    ...
    for left, right in candidates:
        ...
        mode_signal = any(
            _continuous(_tag(left_tags, key)) and _continuous(_tag(right_tags, key)) for key in BURST_KEYS
        )
        times = []
            ...
            evidence.append(
                {
                    ...
                    "signals": [
                        name
                        for name, value in (
                            ("maker_sequence", sequence_signal or filename_signal),
                            ("maker_mode", mode_signal),
                            ("capture_time", time_signal),
                            ("visual_similarity", visual_signal),
                        )
                        if value
                    ],
