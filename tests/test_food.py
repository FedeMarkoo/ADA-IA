...
class FoodTests(unittest.TestCase):
    def test_persists_shopping_and_recipes(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(run({'db_path': db, 'domain': 'shopping', 'action': 'add', 'item': 'leche', 'quantity': '2', 'unit': 'litros'})['ok'])
            self.assertEqual(run({'db_path': db, 'domain': 'shopping', 'action': 'list'})['items'][0]['item'], 'leche')
            run({'db_path': db, 'domain': 'recipes', 'action': 'save', 'name': 'Tortilla', 'ingredients': 'huevo, papa, cebolla'})
    ...
    def test_seeds_recipes_from_markdown_profile(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            profile.write_text('## 3.1 Hamburguesas caseras\n### Ingredientes\n- carne picada\n- sal\n\n### Freezer\n- Muy buena\n', encoding='utf-8')
            db = str(Path(directory) / 'food.db')
