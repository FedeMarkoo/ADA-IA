...
class AgentServiceTests(unittest.TestCase):
    ...
    def test_memory_tool_request_is_bounded_and_strict(self):
        request = Agent._memory_tool_request(
            '{"tool_call":{"name":"memory.search","arguments":{"query":"preferencias","limit":99}}}'
        )
        self.assertEqual(request, {"query": "preferencias", "limit": 3})
        self.assertIsNone(Agent._memory_tool_request('{"tool_call":{"name":"filesystem.write_file"}}'))
