...
class ContextPhotoAgent(SpecialistAgent):
    ...
    def run(self, task):
        ...
        result = VisionAnalyzer.analyze(str(path), context, task.get("config"))
    ...
    def run(self, task):
        ...
        return AgentResult(
            ...
            True,
            {
                ...
                "recommendation": (
                    "aceptar como seleccionada y revelar"
                    if selection_rating >= 3
                    else "revisar antes de seleccionar" if selection_rating == 2 else "rechazar"
                ),
            },
        )
