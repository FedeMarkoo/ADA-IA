...
def _case(category, case_id, name, capability, prompt, must_match, tags=None):
    ...
    return {"id": case_id, "category": category, "name": name, "capability": capability,
            "tags": tags or [capability, "readonly"], "prompt": prompt, "must_match": must_match}
...
HEALTHCHECK_PROMPTS = [
    _case("web", "web_search", "Búsqueda en internet", "web", "Buscá en internet una noticia actual sobre inteligencia artificial. Resumí el hallazgo en dos frases e incluí al menos una fuente o enlace.", [r"(http|fuente|enlace|sitio)", r"(inteligencia artificial|IA)"]),
    _case("web", "web_search_fact", "Verificación de una búsqueda", "web", "Buscá dos fuentes actuales y confiables sobre el precio o la evolución reciente del dólar en Argentina. Compará las fechas y aclarame si hay diferencias.", [r"(fuente|enlace|sitio)", r"(dólar|dolar)", r"(fecha|diferencia|compar)"]),
    _case("photography", "event_photos_report", "Reporte de fotos de eventos", "photography", "Prepará un reporte de las fotos del evento más reciente que encuentres en mi Drive. Indicá evento, cantidad de fotos, aceptadas/rechazadas y si están exportadas. Si no podés acceder, explicá el motivo.", [r"(foto|evento|Drive)", r"(exportad|no puedo|no encontr|acced)"]),
    _case("photography", "event_photos_quality", "Estado de exportación de un evento", "photography", "Revisá el evento de fotos Sofia en Google Drive y decime si tiene originales, seleccionadas y exportadas. Resumí faltantes sin modificar archivos.", [r"(Sofia|evento|Drive)", r"(original|seleccion|exportad|no encontr|acced)"]),
    _case("finance", "financial_operations", "Reporte de operaciones monetarias", "finance", "Dame un reporte actualizado y prudente sobre oportunidades de comprar o vender acciones, cripto o dólares. Compará alternativas, riesgos, horizonte y datos que faltan. No ejecutes ninguna operación ni presentes esto como asesoramiento financiero personalizado.", [r"(compr|vend|mant|alternativ)", r"(riesgo|volatilidad)", r"(no ejecut|no es asesoramiento|financier)"]),
    _case("finance", "financial_sources", "Fuentes para una decisión financiera", "finance", "Buscá información actual sobre una acción, una criptomoneda y el dólar que yo podría evaluar. Para cada uno indicá fuente, fecha, volatilidad y qué debería verificar antes de decidir. No compres ni vendas nada.", [r"(acción|cripto|dólar|dolar)", r"(fuente|fecha)", r"(riesgo|volatilidad|verificar)"]),
    _case("calendar", "next_calendar_event", "Próximo evento del calendario", "calendar", "Decime cuál es mi próximo evento en Google Calendar. Mostrá título, fecha, hora y calendario. Solo lectura; si no hay eventos o falta conexión, decilo claramente.", [r"(próxim|evento|calendar)", r"(fecha|hora|no hay|no encontr|conexi)"]),
    _case("calendar", "calendar_week", "Agenda de los próximos días", "calendar", "Dame un resumen de mis eventos de Google Calendar durante los próximos siete días, ordenados por fecha y hora. No crees ni modifiques eventos.", [r"(evento|calendar)", r"(fecha|hora|semana|días|no hay)"]),
    _case("gmail", "mail_report", "Reporte de mails", "gmail", "Hacé un reporte de mis mails recientes: cantidad relevante, asuntos, remitentes y fechas. No respondas, envíes ni modifiques correos; si no hay acceso, explicalo.", [r"(mail|correo|mails?)", r"(asunto|remitente|fecha|no hay|acceso)"]),
    _case("gmail", "last_email", "Último correo", "gmail", "Decime cuál es mi último correo recibido: asunto, remitente, fecha y un resumen breve. Solo lectura; no abras enlaces ni realices acciones.", [r"(correo|mail)", r"(asunto|remitente|fecha|no encontr|acceso)"]),
    _case("gmail", "gmail_unread", "Correos no leídos", "gmail", "¿Cuántos correos no leídos tengo y cuáles son los tres asuntos más importantes? Solo consultá Gmail, no marques nada como leído.", [r"(correo|mail|Gmail)", r"(no le[ií]d|asunto|no hay|acceso)"]),
    _case("agent", "capabilities_summary", "Resumen de capacidades", "agent", "Explicame qué puede hacer ADA hoy. Organizá la respuesta por herramientas o categorías y distinguí entre consultar, recomendar y ejecutar.", [r"(puede|capacidad|herramienta)", r"(consult|recomend|ejecut)", r"(web|calendar|Gmail|foto)"]),
    _case("agent", "agent_readonly_boundary", "Límites de solo lectura", "agent", "Explicá qué tareas puede consultar ADA y cuáles requieren confirmación o no debe ejecutar automáticamente. No realices ninguna acción.", [r"(consult|leer|solo lectura)", r"(confirm|permiso|no debe|no ejecut)"]),
]
...
class HealthcheckStore:
    ...
    def __init__(self, memory):
        ...
        for name, definition in (("category", "TEXT NOT NULL DEFAULT 'general'"), ("tags", "TEXT NOT NULL DEFAULT '[]'")):
        ...
        for item in HEALTHCHECK_PROMPTS:
            self.conn.execute(
                ...
                (item["id"], item["category"], item["name"], item["capability"], json.dumps(item["tags"]), item["prompt"], json.dumps(item["must_match"])),
            )
            self.conn.execute("UPDATE healthcheck_prompts SET category=?, capability=?, tags=?, prompt=?, criteria=?, name=?, updated_at=CURRENT_TIMESTAMP WHERE id=?", (item["category"], item["capability"], json.dumps(item["tags"]), item["prompt"], json.dumps(item["must_match"]), item["name"], item["id"]))
        self.conn.commit()
        self._lock.release()

    def prompts(self):
        rows = self.conn.execute("SELECT id,category,name,capability,tags,prompt,criteria FROM healthcheck_prompts WHERE enabled=1 ORDER BY category,rowid").fetchall()
        return [{"id": r[0], "category": r[1], "functional_category": functional_category(r[1]), "name": r[2], "capability": r[3], "tags": json.loads(r[4]), "prompt": r[5], "must_match": json.loads(r[6])} for r in rows]
    ...
    def add_prompt(self, item):
        ...
        self.conn.execute("INSERT INTO healthcheck_prompts(id,category,name,capability,tags,prompt,criteria) VALUES (?,?,?,?,?,?,?)", (item["id"].strip(), item["category"].strip(), item["name"].strip(), item["capability"].strip(), json.dumps(item.get("tags") or [item["capability"], "readonly"]), item["prompt"].strip(), json.dumps(item["must_match"])))
        self.conn.commit()

    def save_run(self, run_id, prompt_id, response, evaluation, elapsed, request=None, status=None, status_code=None, model=None, mcps=None, trace=None):
        ...
        self.conn.execute(
            ...
            (run_id, prompt_id, response, json.dumps(evaluation, ensure_ascii=False), elapsed, request, final_status, status_code, model, json.dumps(mcps or [], ensure_ascii=False), json.dumps(trace or [], ensure_ascii=False)),
        )
        self.conn.commit()
    ...
    def _batch_row(row):
        return {
            "run_id": row[0], "total": row[1], "completed": row[2], "passed": row[3],
            "failed": row[4], "status": row[5], "current_prompt_id": row[6],
            "started_at": row[7], "finished_at": row[8], "updated_at": row[9],
            "percent": round(row[2] / max(1, row[1]) * 100, 1),
        }

    def history(self, limit=30):
        rows = self.conn.execute("SELECT run_id,prompt_id,response,evaluation,elapsed_seconds,request,status,status_code,model,mcps,trace,created_at FROM healthcheck_runs ORDER BY id DESC LIMIT ?", (limit,)).fetchall()
        return [{
            "run_id": r[0], "prompt_id": r[1], "response": r[2], "evaluation": json.loads(r[3]),
            "elapsed_seconds": r[4], "request": r[5], "status": r[6], "status_code": r[7],
            "model": r[8], "mcps": json.loads(r[9] or "[]"), "trace": json.loads(r[10] or "[]"), "created_at": r[11],
        } for r in rows]
    ...
    def latest_results(self):
        ...
        for r in rows:
            ...
            results.append({
                "run_id": r[0], "prompt_id": r[1], "response": r[2], "elapsed_seconds": r[4],
                "status": r[5], "status_code": r[6], "model": r[7],
                "mcps": json.loads(r[8] or "[]"), "trace": json.loads(r[9] or "[]"),
                "created_at": r[10], **evaluation,
            })
        return results
...
def evaluate(item, reply, elapsed, error=None):
    ...
    return {"passed": passed, "missing": missing, "reply_chars": len(reply), "elapsed_seconds": round(elapsed, 3), "error": error}
...
def llm_judge(item, reply, endpoint="http://127.0.0.1:11434", model="llama3.2:3b", mcp_evidence=None):
    ...
    payload = json.dumps({"model": model, "prompt": judge_prompt, "stream": False, "format": "json", "options": {"temperature": 0}}).encode()
    req = urllib.request.Request(f"{endpoint.rstrip('/')}/api/generate", data=payload, headers={"Content-Type": "application/json"}, method="POST")
    try:
        ...
        return {"passed": passed, "score": round(score, 3), "issues": result.get("issues") or [], "rationale": str(result.get("rationale") or ""), "source": "llm", "model": model}
    except Exception as exc:
        return {"passed": False, "score": 0.0, "issues": ["No se pudo ejecutar la IA evaluadora."], "rationale": str(exc), "source": "unavailable", "error": str(exc)}
