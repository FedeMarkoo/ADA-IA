...
# Coloquial path aliases → real paths
_PATH_ALIASES = {
    "escritorio": "~/Desktop",
    "desktop": "~/Desktop",
...
def _resolve_path_alias(text: str) -> str | None:
    """Return the real path if the text is a known coloquial alias, else None."""
    clean = text.strip().lower().rstrip(".")
    # Direct alias match
    if clean in _PATH_ALIASES:
        return os.path.expanduser(_PATH_ALIASES[clean])
    # "el escritorio", "del escritorio", "mi escritorio"
    for alias, real in _PATH_ALIASES.items():
        if re.search(r"\b" + re.escape(alias) + r"\b", clean):
            return os.path.expanduser(real)
    # If it looks like an absolute or home path, return it directly
...
class WebChatService:
    ...
    def _payload(self, parsed, action, text):
        ...
        if action == "filesystem":
            ...
            # Try to extract coloquial path from the raw text
    ...
    def handle(self, text, state, lang=None):
        ...
        # If the bot previously asked for a path and user replies with one, resume the pending task
        if getattr(state, "pending_path_action", None):
            resolved = _resolve_path_alias(text)
            if resolved:
                task = dict(state.pending_path_action)
                ...
                # Can't resolve path, clear and fall through
                state.pending_path_action = None
        ...
        if self._needs_path(action, payload):
            # Save task so next message with a path can resume it
