...
def ollama_config_payload(config: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "local_model_exclusive_mode": bool(config.get("local_model_exclusive_mode", True)),
        "local_model_switch_policy": config.get("local_model_switch_policy", "wait"),
        "local_model_switch_timeout_seconds": int(config.get("local_model_switch_timeout_seconds", 30)),
