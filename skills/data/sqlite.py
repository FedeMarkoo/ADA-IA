...
def run(args):
    ...
    try:
        if action in {'status', 'summary', 'resumen', 'estado'}:
            ...
            return {
                'ok': True, 'tool': 'sqlite', 'action': 'status', 'db': str(database),
                'summary': {'carpetas': summary[0], 'total': summary[1], 'buenas': summary[2],
                            'rechazadas': summary[3], 'eliminadas': summary[4], 'movidas': summary[5],
                            'jpg': summary[6], 'jpg_asociados': summary[7], 'videos': summary[8],
                            'editables': summary[9], 'otros': summary[10]},
        ...
        if action in {'report', 'reporte', 'photo_report', 'reporte_fotos'}:
            ...
            collections = []
            for row in rows:
                item = {'formato': row[0] or 'Sin formato', 'contexto': row[1], 'fecha': row[2],
                        'contenido': row[3], 'ruta': row[4], 'total': row[5],
                        'buenas': row[6], 'rechazadas': row[7], 'eliminadas': row[8],
                        'jpg': row[9], 'jpg_asociados': row[10], 'videos': row[11],
                        'editables': row[12], 'otros': row[13]}
                collections.append(item)
                group = formats.setdefault(item['formato'], {'colecciones': 0, 'carpetas': 0,
                                                               'total': 0, 'buenas': 0,
                                                               'rechazadas': 0, 'eliminadas': 0})
                group['colecciones'] += 1
                group['carpetas'] += 1
            ...
            ''').fetchone()
            if unassigned[0]:
                formats['Sin colección'] = {'colecciones': 0, 'carpetas': unassigned[0],
                                             'total': unassigned[1], 'buenas': unassigned[2],
                                             'rechazadas': unassigned[3], 'eliminadas': unassigned[4],
                                             'jpg': 0, 'jpg_asociados': 0, 'videos': 0,
                                             'editables': 0, 'otros': 0}
            return {'ok': True, 'tool': 'sqlite', 'action': 'report', 'db': str(database),
                    'summary': {'carpetas': summary[0], 'total': summary[1], 'buenas': summary[2],
                                'rechazadas': summary[3], 'eliminadas': summary[4], 'movidas': summary[5],
                                'jpg': summary[6], 'jpg_asociados': summary[7], 'videos': summary[8],
                                'editables': summary[9], 'otros': summary[10]},
        ...
        if action in {'structure', 'estructura', 'folders', 'carpetas'}:
            ...
            ''').fetchall()
            return {
                'ok': True, 'tool': 'sqlite', 'action': 'structure', 'db': str(database),
                'count': len(rows), 'collections': [
                    {'formato': row[0], 'contexto': row[1], 'fecha': row[2],
                     'contenido': row[3], 'ruta': row[4]} for row in rows],
            }
